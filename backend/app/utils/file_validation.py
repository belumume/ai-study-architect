...
def get_mime_type(file_content: bytes) -> str:
    """Get MIME type from file content, with fallback to basic detection."""
    try:
        import magic
...
def validate_file_content(
    ...
    if len(file_content) > max_file_size:
        return (
            False,
            f"File exceeds maximum size of {max_file_size / 1024 / 1024:.1f}MB",
            validation_info,
        )
            ...
            for name in zf.namelist():
                ...
                if "external" in name.lower():
                    validation_info["security_warnings"].append(
                        "Document contains external references"
                    )
                ...
                if name.endswith(".xml") or name.endswith(".rels"):
                    try:
                        ...
                        if b"<script" in file_content or b"javascript:" in file_content:
                            validation_info["security_warnings"].append(
                                "Document contains script references"
                            )
