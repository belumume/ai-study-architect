        ...
        for name, service in self.services:
            try:
                ...
                if hasattr(service, "enabled") and not service.enabled:
                    ...
                    api_key = os.getenv(f"{name.upper()}_API_KEY") or os.getenv(
                        "ANTHROPIC_API_KEY" if name == "Claude" else "OPENAI_API_KEY"
                    )
                    if api_key:
                        logger.warning(
                            f"{name} service has API key but reports not enabled - checking property"
                        )
                    else:
    ...
    async def chat_completion(
        ...
        for name, service in self.services:
            try:
                ...
                # For streaming, return the generator directly (no .get() on generators)
                if stream:
                    return result
