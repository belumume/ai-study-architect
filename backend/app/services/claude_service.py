...
class ClaudeService:
    ...
    def __init__(self):
        ...
        self.timeout = httpx.Timeout(
            60.0, connect=5.0
        )  # Claude can take longer for complex reasoning
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    f"{self.base_url}/messages", headers=self._get_headers(), timeout=5.0
                )
    ...
    async def chat_completion(
        ...
        if not claude_messages or claude_messages[0]["role"] != "user":
            claude_messages.insert(0, {"role": "user", "content": "Please help me learn."})
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                response = await client.post(
                    f"{self.base_url}/messages", headers=self._get_headers(), json=payload
                )
    ...
    async def _stream_claude_response(
        ...
        try:
            async with client.stream(
                "POST", f"{self.base_url}/messages", headers=self._get_headers(), json=payload
            ) as response:
                response.raise_for_status()
    ...
    "common_misconceptions": ["misconception1", ...]
        ...
        try:
            ...
            if json_match:
                ...
                analysis = {"summary": response_text, "key_concepts": [], "socratic_questions": []}
