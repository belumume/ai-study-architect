...
class AdaptDifficultyRequest(BaseModel):
    ...
    performance_score: float = Field(
        ..., ge=0.0, le=1.0, description="Performance score between 0 and 1"
    )
...
def chat_with_tutor(
    ...
    try:
        ...
        agent_manager.save_agent(
            str(current_user.id), "lead_tutor", agent, tutor_request.session_id
        )
...
def get_learning_progress(
    ...
    if not agent:
        return {"message": "No active learning session found", "progress": None}

    progress = agent.get_progress_summary()

    return {"message": "Progress retrieved successfully", "progress": progress}
...
def clear_tutor_session(
    ...
    success = agent_manager.delete_agent(
        user_id=str(current_user.id), agent_type="lead_tutor", session_id=session_id
    )

    if success:
        logger.info(f"Cleared tutor session for user {current_user.id}")
        return {"message": "Tutor session cleared successfully", "success": True}

    return {"message": "No active session to clear", "success": False}
