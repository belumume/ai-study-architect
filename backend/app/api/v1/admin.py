...
def verify_admin(current_user: User = Depends(get_current_user)) -> User:
    ...
    if not current_user.is_superuser:
        raise UnauthorizedError(detail="Admin privileges required", error_code="ADMIN_REQUIRED")
    return current_user
...
def rotate_rsa_keys(
    ...
    try:
        ...
        if rotation_result["status"] == "success":
            ...
            raise HTTPException(
                status_code=500, detail=f"Key rotation failed: {rotation_result.get('error')}"
            )
...
def get_keys_info(
    request: Request, admin_user: User = Depends(verify_admin)
...
@limiter.limit("30/minute")
def get_cache_status(request: Request, admin_user: User = Depends(verify_admin)) -> CacheStatus:
    """
...
def clear_model_cache(request: Request, model: str, admin_user: User = Depends(verify_admin)):
    """
...
def clear_cache_pattern(request: Request, pattern: str, admin_user: User = Depends(verify_admin)):
    """
    ...
    """
    try:
        logger.warning(f"Admin {admin_user.email} clearing cache pattern '{pattern}'")
...
@limiter.limit("10/minute")
def list_user_agents(request: Request, user_id: str, admin_user: User = Depends(verify_admin)):
    """
    ...
    try:
        ...
        return {"user_id": user_id, "agent_count": len(agents), "agents": agents}
...
@limiter.limit("3/hour")
def cleanup_expired_agents(request: Request, admin_user: User = Depends(verify_admin)):
    """
